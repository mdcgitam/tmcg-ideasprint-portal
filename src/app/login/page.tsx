...
function LoginContent() {
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-center bg-void px-6 text-center">
      ...
      <p className="mt-3 max-w-sm font-heading text-sm text-ink-muted">
        Only registered participants with a GITAM university Google account can sign in.
      </p>

      {errorMessage && (
        <p className="mt-6 max-w-sm rounded-lg border border-red-500/40 bg-red-500/10 px-4 py-3 font-heading text-sm text-red-300">
          {errorMessage}
        </p>
      )}
      ...
      <button
        ...
        disabled={loading}
        className="mt-8 rounded-full bg-gold px-8 py-3 font-heading text-sm font-medium text-void transition-colors hover:bg-gold-light disabled:opacity-60"
      >
