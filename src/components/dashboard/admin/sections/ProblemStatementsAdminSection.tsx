...
type View = "team" | "analytics" | "manage";
...
export function ProblemStatementsAdminSection({
  ...
  // ── Manage List (raw CRUD, kept for edge-case metadata edits) ───────────
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setFormError(err instanceof DashboardActionError ? err.message : "Something went wrong.");
    } finally {
      setSubmitting(false);
    }
  }
            ...
            <form onSubmit={handleSubmit} className="rounded-xl border border-border bg-surface p-6">
              ...
              <div className="mt-4 grid gap-4 sm:grid-cols-2">
                ...
                <input
                  ...
                  placeholder="Title (optional — the sheet is the source of truth)"
                  className="rounded-lg border border-border bg-void px-4 py-2.5 font-heading text-sm text-ink outline-none focus:border-gold"
                />
                <textarea
              ...
              {formError && <p className="mt-3 font-heading text-sm text-danger">{formError}</p>}
